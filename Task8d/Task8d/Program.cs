...
class Task8d
{
    ...
    static void Main()
    {
        ...
        try
        {
            Console.WriteLine("Insert a name of a person");
            string? FindPerson = Console.ReadLine();
            

            Regex NameRegex = new Regex(string.Format(@"^{0}", FindPerson));


            if (People.Where(P => NameRegex.IsMatch(P.Name)).Count() > 0)
            {
                Person PersonResult = People.Where(P => NameRegex.IsMatch( P.Name)).First();
                PersonResult.Print();
            }
            else 
            { 
                Console.WriteLine("Person '{0}' Could not be found", FindPerson); 
            }    
